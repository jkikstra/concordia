...
model, harm, hist = read_version(settings.version, settings.variable_template)
...
model.pix.unique("model")[-1]

# TODO currently only works for a single model
model_name = model.pix.unique("model")[-1]
...
plot_harm(
    isin(region="MESSAGEix-GLOBIOM 2.1-R12|China", sector="Energy Sector", gas="CH4"),
    scenario="SSP1 - Low Emissions",
)
...
g = plot_harm(
    isin(sector="Agriculture", gas="CH4"),
    scenario="SSP1 - Low Emissions",
    useplotly=False,
)
